  ...
  const directors = [
    {
      ...
      sections: [
        ...
        {
          heading: "Notable Advisory & Investigations",
          bullets: [
        ...
        {directors.map((director) => (
          ...
          >
            ...
            <div className="team-card-header">
              <div
                style={{
                  width: "70px",
                  height: "70px",
                  backgroundColor: "rgba(0, 78, 163, 0.08)",
                  borderRadius: "50%",
                  display: "flex",
                  alignItems: "center",
              ...
              >
                <i className={`fa-solid ${director.icon}`}></i>
              </div>
              <div>
            ...
            <div className="modal-bio-body">
              {director.sections.map((section, idx) => (
                <div key={idx} style={{ marginBottom: "24px" }}>
                  <h4 style={{ fontSize: "15px", textTransform: "uppercase", letterSpacing: "0.5px", color: "var(--primary-dark)", borderBottom: "1px solid var(--border-color)", paddingBottom: "6px", marginBottom: "12px" }}>
                    {section.heading}
                  </h4>
                  {section.content && <p style={{ fontSize: "14px", color: "#444444", lineHeight: "1.6" }}>{section.content}</p>}
                  
                  {section.bullets && (
                    <ul className="check-list" style={{ paddingLeft: "0", listStyle: "none" }}>
                      {section.bullets.map((bullet, i) => (
                        <li key={i} style={{ marginBottom: "8px", display: "flex", gap: "10px", fontSize: "13.5px", color: "#444444" }}>
                          {bullet}
                        </li>
                      ))}
                    </ul>
                  ...
                  {section.table && (
                    <div style={{ overflowX: "auto", marginTop: "12px" }}>
                      <table className="dpo-table" style={{ width: "100%", borderCollapse: "collapse", border: "1px solid var(--border-color)" }}>
                        <thead>
                          <tr>
                            {section.table.headers.map((h, i) => (
                              <th key={i} style={{ padding: "10px", border: "1px solid var(--border-color)", background: "#f8f9fa", fontWeight: "bold", fontSize: "13px", textAlign: "left" }}>
                                {h}
                              </th>
                            ))}
                          </tr>
                        ...
                        <tbody>
                          {section.table.rows.map((row, i) => (
                            <tr key={i}>
                              {row.map((cell, j) => (
                                <td key={j} style={{ padding: "10px", border: "1px solid var(--border-color)", fontSize: "12.5px" }}>
                                  {cell}
                                </td>
                              ))}
                            </tr>
